...
export class ObjectInstanceWrangler {
  ...
  private calculateAdjacentEdgeOffset(siblingElement: Element): Offset {
    ...
    if ([edgeDistances.top, edgeDistances.bottom].includes(smallestDistance)) {
      offset[smallestDistance === edgeDistances.top ? 'top' : 'bottom'] = -smallestDistance / 2 + 'px';
      offset.left = Math.min(0, siblingBounds.left - thisBounds.left) + 'px';
      offset.right = Math.min(0, thisBounds.right - siblingBounds.right) + 'px';
    } else {
      offset[smallestDistance === edgeDistances.left ? 'left' : 'right'] = -smallestDistance / 2 + 'px';
      offset.top = Math.min(0, siblingBounds.top - thisBounds.top) + 'px';
      offset.bottom = Math.min(0, thisBounds.bottom - siblingBounds.bottom) + 'px';
    }
