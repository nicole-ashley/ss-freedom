import { r as registerInstance, h } from './index-70ea5c31.js';
...
class ElementReplacement {
  static replaceObjectWithMostLikelyEquivalent(original, newDocument) {
  ...
  static findMostLikelyEquivalentElement(original, newDocument) {
    const ancestry = [];
  ...
  static generateQueryPartsForElement(element) {
    const parts = [];
  ...
  static queryDocumentFor(document, ancestry) {
...
class TinyMceWrangler {
  constructor(_MutationObserver = MutationObserver, htmlElement = document.documentElement, apiService = null) {
    this.observer = new _MutationObserver((changes) => this.observerCallback(changes));
    this.htmlElement = htmlElement;
    this.apiService = apiService || new ApiService();
    this.getTinyMce();
  }
  static getTinyMceConfigurations() {
  ...
  observeDom() {
    ...
    Array.from(this.htmlElement.querySelectorAll('[ss-freedom-field]')).forEach((element) => this.observeElementIfField(element));
  }
  observerCallback(changes) {
    changes.forEach(async (change) => {
      if (change.type === 'attributes' && change.attributeName === 'ss-freedom-field') {
        this.observeElementIfField(change.target);
      }
        ...
        Array.from(change.addedNodes).forEach((node) => {
          if (node instanceof HTMLElement) {
            ...
            fieldNodes.forEach((object) => this.observeElementIfField(object));
          }
        });
      }
      (await this.tinyMce).editors.forEach((editor) => {
  ...
  async observeElementIfField(element) {
    if (element.hasAttribute('ss-freedom-field')) {
      this.initialiseEditorIfNecessary(element);
      TinyMceWrangler.handleBackgroundColours(element);
    }
  }
  async initialiseEditorIfNecessary(element) {
    const existingEditor = (await this.tinyMce).editors.find((e) => e.getBody() === element);
    if (!existingEditor) {
  ...
  static handleBackgroundColours(element) {
  ...
  static applyNonHoverBackgroundColour(element) {
  ...
  static applyHoverBackgroundColour(element) {
    if (!element.classList.contains('ss-freedom-empty')) {
      TinyMceWrangler.applyBackgroundColour(element, 0.1);
    }
    else {
  ...
  static toCssColor({ fn, c1, c2, c3, a }) {
    return `${fn}a(${c1}, ${c2}, ${c3}, ${a})`;
  }
  async initialiseTinyMceConfiguration(name, element) {
    const configurations = TinyMceWrangler.getTinyMceConfigurations();
    const setups = [];
    ...
    config = Object.assign(config, {
      target: element,
      setup: (editor) => setups.forEach((fn) => fn(editor)),
    ...
    if (config.element_options) {
      ...
      setups.push((editor) => this.createTinyMceOptionsMenu(editor, elementOptions));
    }
    ...
    setups.push((editor) => {
      editor.on('PostProcess', function (e) {
        e.content = e.content.replace(/<ss-freedom-shortcode[^>]*?tag=(["'])(.*?)\1[^>]*?>.*?<\/ss-freedom-shortcode>/g, (_all, _quote, tag) => he.decode(tag));
      });
    });
  ...
  extendConfiguration(base, extension, configurations) {
    ...
    return Object.keys(extension).reduce((output, key) => {
      if (Array.isArray(output[key])) {
        ...
        output[key] = [].concat(extension[key].slice(0, Math.max(0, existingPosition)), output[key], extension[key].slice(existingPosition + 1));
      }
  ...
  createTinyMceOptionsMenu(editor, configurations) {
  ...
  async updateCmsWithLatestData(editor) {
...
class ObjectInstanceWrangler {
  constructor(element) {
    this.adjacentSiblingButtons = {};
  ...
  handleUpdate(e = null) {
    ...
    const monitoredElements = [
      ...
      this.alertButton,
      this.deleteButton,
      (_a = this.adjacentSiblingButtons) === null || _a === void 0 ? void 0 : _a.before,
      (_b = this.adjacentSiblingButtons) === null || _b === void 0 ? void 0 : _b.after,
      ...this.emptyRelationAddItemButtons,
      ...Array.from(document.querySelectorAll(`ss-freedom-object-options-panel[ss-freedom-uid="${metadata.uid}"]`))
    ];
  ...
  ensureButtonsPresent() {
    ...
    if ((metadata.canDelete || relationMetadata.removeMethod === 'unlink') &&
  ...
  showAdjacentAddButtonsIfInList() {
    if (this.element.parentElement.hasAttribute('ss-freedom-relation') &&
  ...
  discoverClosestListSiblingsAdjacentEdges() {
    const edges = {};
  ...
  calculateAdjacentEdgeOffset(siblingElement) {
    ...
    const offset = {};
    if (((thisBounds.top > siblingBounds.bottom || thisBounds.bottom < siblingBounds.top) &&
  ...
  showEmptyRelationAddItemButtons() {
    const emptyRelations = this.element.querySelectorAll('[ss-freedom-relation]:empty');
    emptyRelations.forEach((relation) => {
      const relationBounds = relation.getBoundingClientRect();
      const offset = {};
      if (relationBounds.width > relationBounds.height) {
        offset.top = offset.right = offset.left = '0rem';
      }
      else {
        offset.right = offset.bottom = offset.left = '0rem';
      }
      const addItemButton = document.createElement('ss-freedom-add-item-button');
      addItemButton.element = relation;
      addItemButton.offset = offset;
      this.emptyRelationAddItemButtons.push(addItemButton);
      document.body.prepend(addItemButton);
    });
  }
  hideEmptyRelationAddItemButtons() {
    while (this.emptyRelationAddItemButtons.length) {
      const button = this.emptyRelationAddItemButtons.pop();
      button.remove();
    }
  }
  showEmptyFields() {
...
class ObjectWrangler {
  constructor(_MutationObserver = MutationObserver, htmlElement = document.documentElement) {
    this.observer = new _MutationObserver((changes) => this.observerCallback(changes));
    this.htmlElement = htmlElement;
  }
  observeDom() {
    ...
    Array.from(this.htmlElement.querySelectorAll('[ss-freedom-object]'))
      .forEach((element) => this.wireObjectOptionsIfNecessary(element));
  }
  observerCallback(changes) {
    changes.forEach((change) => {
      if (change.type === 'attributes' && change.attributeName === 'ss-freedom-object') {
        this.wireObjectOptionsIfNecessary(change.target);
      }
      else if (change.type === 'childList') {
        Array.from(change.addedNodes).forEach((node) => {
          if (node instanceof HTMLElement) {
            ...
            objectNodes.forEach(object => this.wireObjectOptionsIfNecessary(object));
          }
        });
        Array.from(change.removedNodes).forEach((node) => {
          if (node instanceof HTMLElement) {
            ...
            objectNodes.forEach(object => this.unwireObjectOptionsIfPresent(object));
          }
        });
      }
    });
  }
  wireObjectOptionsIfNecessary(element) {
    if (!element['ssFreedomObjectInstanceWrangler']) {
      element['ssFreedomObjectInstanceWrangler'] = new ObjectInstanceWrangler(element);
    }
  }
  unwireObjectOptionsIfPresent(element) {
    if (element['ssFreedomObjectInstanceWrangler']) {
...
const adminWidgetCss = "*{font-family:-apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, Oxygen-Sans, Ubuntu, Cantarell, \"Helvetica Neue\", sans-serif}a{color:#207ab7;transition:color 300ms;text-decoration:none}a:hover{color:#1c6ca1;text-decoration:underline}a:active{color:#185d8c}*{box-sizing:border-box}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}div{--easing:cubic-bezier(0.165, 0.84, 0.44, 1);position:fixed;z-index:2;bottom:1rem;left:1rem;width:auto;height:auto;box-sizing:content-box;box-shadow:0 0 1rem rgba(0, 0, 0, 0.1);border:solid 1px;overflow:hidden;transition-property:border-color, border-radius, background-color, padding, max-width, max-height;transition-duration:500ms;transition-timing-function:var(--easing)}div.closed{max-width:4rem;max-height:4rem;border-color:transparent;border-radius:50%;background-color:#004a7a;padding:0}div.closed form{opacity:0;pointer-events:none}div.open{border-color:lightgrey;border-radius:3px;background-color:white;padding:2rem 1rem 0.5rem;max-width:8rem;max-height:11rem}div.open button.silverstripe{opacity:0;pointer-events:none}button.silverstripe,form{transition:opacity 500ms var(--easing)}button.silverstripe{position:absolute;color:white;width:4rem;height:4rem;top:50%;left:50%;transform:translate(-50%, -50%);font-size:2rem;appearance:none;background:none;border:0;outline:0;padding:0;display:table-cell;vertical-align:middle;text-align:center}button.silverstripe:before{display:block;content:\"M\";font-family:silverstripe;font-style:normal;font-weight:400;font-variant:normal;text-transform:none;speak:none;line-height:1;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}form>*{display:block;margin-bottom:0.5rem}form label{color:rgba(34, 47, 62, 0.7);font-size:0.875rem}form select{border:solid 1px #ccc;border-radius:3px;padding:0.31rem 0.29rem;font-size:1rem;display:block}form button.publish,form button.published{position:relative}form button.publish ion-icon,form button.published ion-icon{position:absolute;top:calc(50% - 0.5em);left:calc(50% - 0.5em);display:none}form button.publish{appearance:none;border-radius:3px;border:none;padding:0.25rem 1rem;font-size:1rem;font-weight:bold;line-height:1.5rem;transition:background-color 300ms}form button.publish:disabled{color:rgba(0, 0, 0, 0.3);background-color:#f0f0f0}form button.publish:not(:disabled){color:#fff;background-color:#207ab7}form button.publish:not(:disabled):hover{background-color:#1c6ca1}form button.publish:not(:disabled):active{background-color:#185d8c}form button.published{appearance:none;border-radius:3px;border:none;padding:0.25rem 1rem;font-size:1rem;font-weight:bold;line-height:1.5rem;transition:background-color 300ms}form button.published:disabled{color:rgba(0, 0, 0, 0.3);background-color:#f0f0f0}form button.published:not(:disabled){color:#222f3e;background-color:#f0f0f0}form button.published:not(:disabled):hover{background-color:#e3e3e3}form button.published:not(:disabled):active{background-color:#d6d6d6}form button.publishing span{color:transparent}form button.publishing ion-icon{display:block;animation:1s infinite linear spin}form .settings,form .close{position:absolute;top:0rem;width:2.5rem;height:2.5rem;appearance:none;border:0;background:none}form .settings{right:2.5rem}form .close{right:0}";

const AdminWidget = class {
  constructor(hostRef) {
  ...
  static async RefreshPublishedStatus() {
    document.querySelectorAll('ss-freedom-admin-widget')
      .forEach((widget) => widget.refreshPublishedStatus());
  }
  componentWillLoad() {
    if (this.stage !== 'Live') {
  ...
  render() {
    return (h("div", { class: this.isOpen ? 'open' : 'closed' }, h("form", null, h("label", null, "Stage:", h("select", { ref: el => this.stageDropdown = el, onChange: () => this.changeStage() }, h("option", { selected: this.stage === 'Stage', value: "Stage" }, "Staging"), h("option", { selected: this.stage === 'Live', value: "Live" }, "Live"))), this.cmsEditLink &&
      h("a", { href: this.cmsEditLink }, "Edit in CMS"), this.stage !== 'Live' && this.canPublish &&
      h("button", { type: "button", class: this.isPublished ? 'published' : 'publish', onClick: e => this.publishPage(e.target) }, h("ion-icon", { class: "loading", name: "refresh" }), h("span", null, this.isPublished ? 'Published' : 'Publish')), h("button", { type: "button", class: "close", onClick: () => this.close() }, h("ion-icon", { name: "close" }))), h("button", { class: "silverstripe", onClick: () => this.open() })));
  }
};
AdminWidget.style = adminWidgetCss;

export { ApiService as A, ElementReplacement as E, AdminWidget as a };
